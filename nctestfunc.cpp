	...
	float* t_inp, float *fillVal, const char* ws_FileName, const char* ws_VarName, const char* yName, const char* xName, int nZones, const char * zName, float* y_inp, float* x_inp)
{
	...
	if ((retncval = nc_open(ws_FileName, NC_NOWRITE, &ncid)))                  //NC_MPIIO, MPI::COMM_WORLD, MPI::INFO_NULL, &ncid)))
		ERR(retncval);
	...
	size_t start[NDIMS], count[NDIMS];
	// Create netcdf file.  
	if ((retncval = nc_create(FileName, NC_NETCDF4 | NC_CLOBBER, &ncid_out)))              //NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, MPI::COMM_WORLD, MPI::INFO_NULL, &ncid_out)))
		ERR(retncval);
	//?? set fill on
