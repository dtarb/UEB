...
__host__ __device__ int readNC_yxSlub(const char* FILE_NAME, const char* VAR_NAME, const char* tcor_NAME, int &tStart, int tEnd,
	float*** &pvar_in, int &nrecords, int &numNc, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	if ((retncval = nc_open_par(FILE_NAME, NC_MPIIO, inpComm, inpInfo, &ncid)))
		ERR(retncval);
	// get variable id
	...
	if (retncval = nc_var_par_access(ncid, pvarid, NC_COLLECTIVE))
		ERR(retncval);
	...
	int Nydim, int Nxdim, float*** pvar_in, int &nrecords, int &numNc, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	for (int i = 0; i < 3; i++)	{
		...
		if (strcmp(pdim_Names, tcor_NAME) == 0){
			...
			if (tEnd < pdim_sizes){
				count[i] = 1;
				tDim = tEnd - tStart;
				tStart += tDim;            // count[i];                //new start point
			}
			else{
				count[i] = 1;
				tDim = pdim_sizes - tStart;        //take the lower of the tEnd/count[i] to guarantee against going out of bounds
				numNc++;                               //next time go to the next netcdf file
				tStart = 0;
			}
			nrecords = tDim;        // count[i];
		}
	...
	{
		start[tIndx] += tk;//read var data
		if (retncval = nc_get_vara_float(ncid, pvarid, start, count, &pvar_in[tk][0][0]))
			ERR(retncval);
	}
	//close netcdf file			
	if (retncval = nc_close(ncid))
	...
	float* tcorvar, float* ycorvar, float* xcorvar, size_t &Ntdim, size_t &Nydim, size_t &Nxdim, float* &pvar_in, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	if (retncval = nc_var_par_access(ncid, pvarid, NC_COLLECTIVE))
		ERR(retncval);
	if (retncval = nc_var_par_access(ncid, ptid, NC_COLLECTIVE))
		ERR(retncval);
	if (retncval = nc_var_par_access(ncid, pyid, NC_COLLECTIVE))
		ERR(retncval);
	if (retncval = nc_var_par_access(ncid, pxid, NC_COLLECTIVE))
		ERR(retncval);
...
__host__ __device__ int WriteTSto3DNC(const char* FileName, const char* VarName, int dimOrd, int y_dim, int x_dim, int Nt_dim, float* var_inp, MPI::Intracomm inpComm, MPI::Info inpInfo)  //ydim, xdim =the coordinate point the data to be written; Nt_dim =the length of the TS
{
	...
	// Open netcdf file.  
	if (retncval = nc_open_par(FileName, NC_WRITE | NC_MPIIO, inpComm, inpInfo, &ncid))
		ERR(retncval);
	// get variable id
...
__host__ __device__ int WriteTSto3DNC_Block(const char* FileName, const char* VarName, int dimOrd, int *YindArr, int *XindArr, int bSize, int Nt_dim, float** var_inp, MPI::Intracomm inpComm, MPI::Info inpInfo)  //ydim, xdim =the coordinate point the data to be written; Nt_dim =the length of the TS
{
	// IDs for the netCDF file, dimensions, and variables. 
	int ncid = 0, y_dimid = 0, x_dimid = 0, t_dimid = 0;
	int v_varid = 0, t_varid = 0, y_varid = 0, x_varid = 0;
...
__host__ __device__ int Write_uebaggTS_toNC_par(const char* FileName, const char* VarName, int dimOrd, int z_dim, int Nt_dim, float* var_inp, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	//parallel access mode
	if ((retncval = nc_var_par_access(ncid, v_varid, NC_INDEPENDENT)))
		ERR(retncval);
	//put variable values
	if (retncval = nc_put_vara_float(ncid, v_varid, start, count, &var_inp[0]))
		ERR(retncval);
...
float* t_inp, float *fillVal, const char* ws_FileName, const char* ws_VarName, const char* yName, const char* xName, int nZones, const char * zName, float* y_inp, float* x_inp, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	size_t start[NDIMS], count[NDIMS];
	// Create netcdf file.  
	if ((retncval = nc_create_par(FileName, NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, inpComm, inpInfo, &ncid_out)))
		ERR(retncval);
	//?? set fill on
	if ((retncval = nc_set_fill(ncid_out, NC_FILL, &oldFill)))
		ERR(retncval);
	...
	//parallel access mode
	if ((retncval = nc_var_par_access(ncid_out, v_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid_out, t_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid_out, y_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid_out, x_varid, NC_COLLECTIVE)))
		ERR(retncval);
	//put values to dim variables
	if (retncval = nc_put_var_float(ncid_out, t_varid, &t_inp[0]))
		ERR(retncval);
	...
	const char* tcalendar, int Nt_dim, int dimOrd, float* t_inp, float *fillVal, const char* ws_FileName, const char* ws_VarName, const char* yName, const char* xName, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	//parallel access mode
	/*if ((retncval = nc_var_par_access(ncid, pvarid,NC_INDEPENDENT)))
	ERR(retncval);*/
	if ((retncval = nc_var_par_access(ncid, pyid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, pxid, NC_COLLECTIVE)))
	...
	const char* yUnits, const char* xUnits, int Nt_dim, int Ny_dim, int Nx_dim, int dimOrd, float* t_inp, float* y_inp, float* x_inp, float* fillVal, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	int retncval = 0;
	// Create netcdf file.  
	if ((retncval = nc_create_par(FileName, NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, inpComm, inpInfo, &ncid)))
		ERR(retncval);
	//set fill on
	...
	if ((retncval = nc_var_par_access(ncid, v_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, t_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, y_varid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, x_varid, NC_COLLECTIVE)))
		ERR(retncval);
	//put values to dim variables
...
//function to read single column/rod variable and time array along time dimension from 3D netcdf file, for given y , x coordinates
...
__host__ __device__ int readNC_TS(const char* FILE_NAME, const char* VAR_NAME, const char* tcor_NAME, const char* ycor_NAME, const char* xcor_NAME,
	float* &pvar_in, int ydim, int xdim, int &nrecords, MPI::Intracomm inpComm, MPI::Info inpInfo) //, int xstride) //double* tcorvar, double*** pvar_in)
{
	//6.24.14
	int xdimindx = 1;
	float* pvarin_temp = NULL;
	...
	const char* ycor_NAME, const char* tcor_NAME, float* xcorvar, float* ycorvar, float* tcorvar, float*** pvar_out, MPI::Intracomm inpComm, MPI::Info inpInfo)
{
	...
	if ((retncval = nc_create_par(FILE_NAME, NC_NETCDF4 | NC_MPIIO | NC_CLOBBER, inpComm, inpInfo, &ncid)))
		ERR(retncval);
	...
	*/
	...
	if ((retncval = nc_var_par_access(ncid, pvarid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, ptvarid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, pyvarid, NC_COLLECTIVE)))
		ERR(retncval);
	if ((retncval = nc_var_par_access(ncid, pxvarid, NC_COLLECTIVE)))
		ERR(retncval);
